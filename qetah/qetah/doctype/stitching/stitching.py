...
class Stitching(Document):
	def validate(self):
		...
		if(self.stitching_table):
			pass
		else:
			frappe.throw("Stitching Table is Mandorary")

		dd = frappe.get_doc("Sales Order",self.sales_order)
		dd.workflow_state = 'Assign For Stitching'
		dd.save()
	...
	def on_submit(self):
		...
		for i in self.stitching_table:
			...
			user_id = user
			notification_subject = f'{item} Stitching is Completed - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty}, Completed by {user}'
	...
	# 		total_qty = i.total_qty
	# 		if(i.status == 'Accept'):
	# 			notification_subject = f'Item Code - {item} - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty} - Total Qty - {total_qty} is Accepted for Stitching'
	# 			notification_message = 'Stitching Status.'
	# 			notification = frappe.get_doc({
	# 			"doctype": "Notification Log",
	# 			"subject": notification_subject,
	# 			"email_content": notification_message,

...
frappe.ui.form.on('Ironing', {
	refresh: function(frm) {
		frm.set_df_property('ironing_table', 'cannot_add_rows', true);
		frm.set_df_property('ironing_table', 'cannot_delete_rows', true);
		frm.set_df_property('ironing_table', 'cannot_delete_all_rows', true);
		...
		frm.set_value('naming_series',"IRON-ITEMS-")
		frm.refresh_field("naming_series")
		// if(frm.doc.docstatus == 0)
		// {
		// 	frm.add_custom_button(__('Send Notification'), () => {
		// 		frappe.model.open_mapped_doc({
		// 			method: "tailor_shop.tailor_shop.doctype.assigning_items.assigning_items.ironing_notify",
		// 			frm: cur_frm
		// 		})
		...
		{
			frm.add_custom_button(__('Create Packing'), () => {
				frappe.model.open_mapped_doc({
					method: "tailor_shop.tailor_shop.doctype.assigning_items.assigning_items.create_packing",
					frm: cur_frm
				})
			});
		}
		...
		frm.fields_dict['sales_items'].grid.add_custom_button('Add items', () => {	
			$.each(frm.doc.sales_items, function (index, source_row) {
			        ...
			        {
			        var childTable = cur_frm.add_child("ironing_table");
					...
					frm.refresh_fields("ironing_table");
			        }
				});
		});
	},

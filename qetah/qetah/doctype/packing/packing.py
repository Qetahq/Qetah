...
class Packing(Document):
	def validate(self):
		...
		if(self.packing_table):
			pass
		else:
			frappe.throw("Packing Table is Mandorary")
		dd = frappe.get_doc("Sales Order",self.sales_order)
		dd.workflow_state = 'Assign For Packing'
		dd.save()
	...
	def on_submit(self):
		dd = frappe.get_doc("Sales Order",self.sales_order)
		dd.workflow_state = 'Packing Completed'
		dd.save()
		sales_total=0
		assign_total=0
		for i in self.packing_table:
		...
		for i in self.packing_table:
			...
			user_id = user
			notification_subject = f'{item} Packing is Completed - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty}, Completed by {user}'
	...
	# 		total_qty = i.total_qty
	# 		if(i.status == 'Accept'):
	# 			notification_subject = f'Item Code - {item} - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty} - Total Qty - {total_qty} is Accepted for Packing'
	# 			notification_message = 'Packing Status.'
	# 			notification = frappe.get_doc({
	# 			"doctype": "Notification Log",
	# 			"subject": notification_subject,
	# 			"email_content": notification_message,

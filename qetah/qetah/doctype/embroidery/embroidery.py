...
class Embroidery(Document):
	def validate(self):
		lst = []
		item = []
		for i in self.embroidery_table:
		...
		if(self.embroidery_table):
			pass
		else:
			frappe.throw("Embroidery Table is Mandorary")
		dd = frappe.get_doc("Sales Order",self.sales_order)
		dd.workflow_state = 'Assign For Embroidery'
		dd.save()

	def on_trash(self):
		dd = frappe.get_doc("Sales Order",self.sales_order)
		dd.workflow_state = 'Stitching Completed'
		dd.save()
	...
	def on_submit(self):
		...
		for i in self.embroidery_table:
			...
			user_id = user
			notification_subject = f'{item} Embroidery is Completed - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty}, Completed by {user}'
	...
	# 		total_qty = i.total_qty
	# 		if(i.status == 'Accept'):
	# 			notification_subject = f'Item Code - {item} - Normal Qty - {normal_qty} - Urgent Qty - {urgent_qty} - Total Qty - {total_qty} is Accepted for Embroidery'
	# 			notification_message = 'Embroidery Status.'
	# 			notification = frappe.get_doc({
	# 			"doctype": "Notification Log",
	# 			"subject": notification_subject,
	# 			"email_content": notification_message,
